...
def TDF_Loss(outputs_target, prototypes, cls_normalization=False,proto_detach = False,t = 2.5 ):
    """
    Compute the TDF loss based on target outputs and prototypes.
    ...
    TDF_loss = riemannian_distance(cov_matrix_t, cov_matrix_p)
    return TDF_loss

def mdd_adv_loss(source_logits,target_logits,source_logits_adv,target_logits_adv,srcweight=3):
    class_criterion = nn.CrossEntropyLoss()
    # target_adv = outputs.max(1)[1]
    target_adv_src = source_logits.max(1)[1]
    target_adv_tgt = target_logits.max(1)[1]

    classifier_loss_adv_src = class_criterion(source_logits_adv, target_adv_src)

    logloss_tgt = torch.log(1 - F.softmax(target_logits_adv, dim = 1))
